...
- **车机优化**:大控制键、深色高对比界面,卡拉 OK 式滚动歌词;居中与左右分栏双布局、横向划动切歌,适合行车操作。
...
- 播放页双布局(居中 / 左右分栏);分栏可拖动分隔条调节歌词区宽度,歌词字号随宽度自动缩放
- 驾驶友好:横向划动切歌(左→右下一首、右→左上一首)、加大的控制键与迷你播放条按钮
- 主题:五套配色,并可「跟随日出日落」按真实日出日落自动切换深 / 浅色
- 专辑页与歌手页、播放队列、红心收藏
...
- **公网地址进入即为你的登录态**,建议用 Cloudflare Access 等加一道登录门禁(见 [DEPLOY.md](DEPLOY.md) 的「安全」一节)。
